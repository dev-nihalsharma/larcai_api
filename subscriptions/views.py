...
class BuyCreditSerializer(serializers.Serializer):
    ...
    def validate_credits(self, value):
        ...
        if total < MIN_ORDER_INR:
            raise serializers.ValidationError(
                f"Minimum purchase is ₹{MIN_ORDER_INR} (incl. GST); "
                f"try at least {int(MIN_ORDER_INR / (1 + GST_RATE)) + 1} credits."
            )
        return value
...
def buy_credits(request):
    ...
    subtotal_inr = float(credits)
    gst_inr = round(subtotal_inr * GST_RATE, 2)
    amount = round(subtotal_inr + gst_inr, 2)

    # 2. Generate order id (embed credits for success callback crediting)
    order_id = f"CREDIT_{user.id}_{credits}_{uuid.uuid4().hex[:6]}"

    # 3. Cashfree payload
    host = os.getenv(
        "DEBUG") == "True" and "http://localhost:8000" or os.getenv("PROD_HOST")
    ...
    payload = {
        ...
        "order_meta": {
            "return_url": f"{host}/subscriptions/success?order_id={order_id}"
        }
    }
